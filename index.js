...
const encontrarNumeroMayor = (array) => {
    const numeroMayor = array.reduce((acc,curr)=>{
        if(acc<curr){
            acc = curr
        }
        return acc
    },0) //para obtener el menor agarro el primero(array[0])
    return numeroMayor
  }
...
// 2 Una vez resuelto el ejercicio anterior, vamos a complicarlo. Dado un array de objetos, hacé la función encontrarNumeroMayorEnArrayDeObjetos que reciba un array de objetos y retorne el objeto que tiene el mayor número en la propiedad monto. La estrategia deberia ser similar al ejercicio anterior. Pista: recordá que en un reduce, si no le pasas un valor inicial a la acumuladora, la acumuladora tendrá el valor del primer elemento del array.
 ...
 const encontrarNumeroMayorEnArrayDeObjetos = (array) => {
  const objetoDeNumeroMayor = array.reduce((acc,curr)=>{
      if(acc.monto<curr.monto){
        acc = curr
      }
      return acc
  })
  return objetoDeNumeroMayor
 }
...
const obtenerObjetoConMayorGanancia = (array) => {
    ...
    const objetoConMayorGanancia = objetosGanancia.reduce((acc,curr)=>{
        if(acc.monto<curr.monto){
            acc = curr
        }
        return acc
...
// 4 Una vez resuelto el ejercicio anterior, declará la función obtenerCategoriaConMayorGanancia que reciba un array de objetos y retorne sólo la propiedad categoria del objeto que tenga el mayor monto pero sólo si su tipo es ganancia. La estrategia debería ser similar a la del ejercicio anterior. Pista: recordá que podés usar el operador && adentro de un if para encadenar varias condiciones.
//  usá los mismos arrays de objetos que usamos en el ejercicio anterior

const obtenerCategoriaConMayorGanancia = (array) => {
    const objetoTipoGanancia = array.filter((curr)=>{
        return curr.tipo === "ganancia"
    })
    const objetoGananciaMayor = objetoTipoGanancia.reduce((acc,curr)=>{
        if(acc.monto<curr.monto){
            acc = curr 
        }
        return acc
    ...
    return objetoGananciaMayor.categoria
}
